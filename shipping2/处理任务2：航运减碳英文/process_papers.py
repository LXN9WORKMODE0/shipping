...
def calculate_score(paper_info):
    ...
    # 3. 引用次数评分 (20%) - 替代原来的文献类型评分
    citations = paper_info.get('引用次数', 0)
    if citations >= 50:
        citation_score = 20
    elif citations >= 20:
        citation_score = 15
    ...
    return {
        ...
        'citation_score': citation_score,  # 改名为 citations_score 更清晰
...
## 评估要求：
1. 判断论文是否与该研究方向相关（完全相关=50分，部分相关=25分，不相关=0分）- 权重最高
2. 评估引用次数价值（50次以上=20分，20-49次=15分，10-19次=12分，5-9次=8分，<5次=5分）
...
def parse_api_response(response_text):
    ...
    # 提取引用得分（原类型得分）
    match = re.search(r'引用得分[：:]\s*(\d+)', response_text)
    if match:
        result['citation_score'] = int(match.group(1))
    ...
    result['total_score'] = result['relevance_score'] + result['citation_score'] + result['abstract_score']
...
def parse_md_file(file_path):
    ...
    # 提取引用次数
    match = re.search(r'\*\*引用次数[：:]\s*(\d+)', content)
    if match:
        paper_info['引用次数'] = int(match.group(1))
...
def load_existing_results(results_d1, results_d2, results_d3, processed_set):
    ...
    if RESULT_FILE_1.exists():
        ...
        for line in content.split('\n'):
            ...
            elif line.startswith('  - 引用次数:'):
                match = re.search(r'引用次数:\s*(\d+)', line)
                if match:
                    current_eval['citation_score'] = int(match.group(1))
...
def save_results(results_d1, results_d2, results_d3):
    ...
    with open(RESULT_FILE_1, 'w', encoding='utf-8') as f:
        ...
        f.write("- 相关性得分(50%) + 引用次数得分(20%) + 摘要质量得分(15%)\n\n")
        f.write("---\n\n")
        ...
        for item in sorted(results_d1, key=lambda x: x['eval'].get('total_score', 0), reverse=True):
            ...
            f.write(f"  - 引用次数: {eval_data.get('citation_score', 0)}分\n")
